...
func CompileSource(ctx context.Context, gh *github.Client, execDir, cacheDir, platform string, config types.BuildConfig, absolute bool) (problems types.BuildProblems, result types.BuildResult, err error) {
	...
	return CompileWithCommand(cmd, config.WorkingDir, absolute)
}
...
func CompileWithCommand(cmd *exec.Cmd, workingDir string, absolute bool) (problems types.BuildProblems, result types.BuildResult, err error) {
	var (
	...
	go func() {
		...
		for scanner.Scan() {
			...
			if len(groups) == 5 {
				...
				if !absolute {
					pwd, _ := os.Getwd()
					problem.File = strings.TrimPrefix(problem.File, pwd + string(filepath.Separator))
				}
