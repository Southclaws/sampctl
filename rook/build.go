...
func Build(ctx context.Context, gh *github.Client, auth transport.AuthMethod, pkg *types.Package, build, cacheDir, platform string, ensure, dry bool, absolute bool, buildFile string) (problems types.BuildProblems, result types.BuildResult, err error) {
	...
	if dry {
		...
		problems, result, err = compiler.CompileWithCommand(cmd, config.WorkingDir, absolute)
		if err != nil {
...
func BuildWatch(ctx context.Context, gh *github.Client, auth transport.AuthMethod, pkg *types.Package, build, cacheDir, platform string, ensure bool, buildFile string, absolute bool, trigger chan types.BuildProblems) (err error) {
	config := GetBuildConfig(*pkg, build)
	if config == nil {
		err = errors.Errorf("no build config named '%s'", build)
		return
...
loop:
	for {
		...
		case event := <-watcher.Events:
			...
			go func() {
				running.Store(true)
				problems, _, err = compiler.CompileSource(ctxInner, gh, pkg.Local, cacheDir, platform, *config, absolute)
				running.Store(false)
